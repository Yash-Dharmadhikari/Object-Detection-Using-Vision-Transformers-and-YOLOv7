# -*- coding: utf-8 -*-
"""ViT_For_Object_Detection.ipynb
...
path_images = "/content/101_ObjectCategories/car_side/"
path_annot = "/content/Annotations/car_side/"

# path_to_downloaded_file = keras.utils.get_file(
#     fname="caltech-101",
#     origin="/Users/yashdharmadhikari/Downloads/caltech-101",
#     extract=True,
...
shutil.unpack_archive("/content/101_ObjectCategories.tar.gz")
shutil.unpack_archive("/content/Annotations.tar")
...
def run_experiment(model, learning_rate, weight_decay, batch_size, num_epochs):
    ...
    checkpoint_filepath = "/content/logs/cp.ckpt"
...
vit_object_detector.save("vit_car16.h5", save_format="h5")
...
"""**Models Testing and graphs**"""

vit_object_detector.save("vit_car16")

# To calculate IoU (intersection over union, given two bounding boxes)
vit_object = keras.models.load_model("vit_car16")
def bounding_box_intersection_over_union(box_predicted, box_truth):
    # get (x, y) coordinates of intersection of bounding boxes
    top_x_intersect = max(box_predicted[0], box_truth[0])
    top_y_intersect = max(box_predicted[1], box_truth[1])
...
for input_image in x_test[:10]:
    ...
    input_image = np.expand_dims(input_image, axis=0)
    preds = vit_object.predict(input_image)[0]
    ...
    mean_iou += bounding_box_intersection_over_union(box_predicted, box_truth)
    bound_box_list.append(bounding_box_intersection_over_union(box_predicted, box_truth))
    # Create the bounding box
    rect = patches.Rectangle(
        (top_left_x, top_left_y),
        bottom_right_x - top_left_x,
...
df = pd.DataFrame(history.history, columns=['loss','val_loss'])
df.head()

sns.lineplot(data=df.head(50),palette="flare",sizes=(.25, 2.5))
plt.grid()
...
for i in range(0, len(image_paths)):
    ...
    image1 = keras.utils.load_img(path_images + image_paths[i],)
    image_size=224
    ...
    image1 = image1.resize((image_size, image_size))
      ...
      # convert image to array and append to list
    images1.append(keras.utils.img_to_array(image1))
x_test1 =np.asarray(images1)
for input_image in x_test1:
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 15))
    im = input_image
...
print("mean_iou: " + str(mean_iou / len(x_test1)))
plt.show()
plt.imshow(images1[1].astype("uint8"))
vit_object_detector.predict(images1[1])
